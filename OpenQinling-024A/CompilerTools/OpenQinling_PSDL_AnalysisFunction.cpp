...
QList<CodeSegment> splitCodeSegment(QList<MiddleNode> &codeList){
    ...
    for(int i = 0;i<codeList.length();i++){
        ...
        if(codeList[i].nodeType == "LABEL"){
            ...
            if(tmp.length()!=0){
                tmp.isHaveBeginLabel =  !thisBlockBeginLabel.isEmpty();
...
QList<CodeOperand> getCodeOperand(MiddleNode &code,QList<VarUse> &allFunValue){
    ...
    if(codeType=="ADD"||codeType=="SUB"||codeType=="MUL"||codeType=="DIV"||codeType=="REM"||
       ...
       codeType=="ITF"||codeType=="UITF"||codeType=="FTI"||codeType=="BTD"||codeType=="WTD"){
        ...
        return oper;
    }else if(codeType=="PUSHARRAY" || codeType=="JNE" || codeType=="JMP" || codeType=="LABEL" || codeType=="INT"){
        tmp.rw = 0;
        tmp.operText = code.args[0];
        tmp.isValue = 0;
        oper.append(tmp);
...
public:
    ...
    struct OrderOpersSegs:public QList<OrderOper>{
        ...
        QString toString(){
            QString tmp = "--------------------\r\n";
            if(isHaveBeginLabel){
                tmp+=":::BeginLabel["+blockBeginLabelName+"]:::\r\n";
            }else{
                tmp+=":::NotHaveBeginLabel:::\r\n";
            }
            if(isHaveGoto){
                tmp+=":::AllGotoLabel["+blockAllGotoLabels.toString()+"]:::\r\n";
                tmp+=":::EndGotoLabel["+blockEndGotoLabel+"]:::\r\n";
            }
    ...
    QList<MiddleNode> generateSegLowCode(OrderOpersSegs&segOrders,int segIndex,bool &status){
        ...
        for(int j = 0;j<regs.length();j++){
            if(regs[j].isUsing&&regs[j].isUpdate){
                bool isStoreToStack = checkVarIsStoreToStack(regs[j].useVar->varName,segIndex,segOrders.length()-1);
                if(isStoreToStack){
                    prefixOper.append(generateLoadStoreValueCode(
                                               1,
                                               regs[j].useVar->varName,
...
QList<MiddleNode> generateCall64bitOperFunCode(QString funName,
    ...
    for(int i = 0;i<varUseInfo.length();i++){
        if(varUseInfo[i].varName == returnPtrName){
            varUseInfo[i].isRetrievedAddress = 1;
            break;
        }
    }
            ...
            for(int j = 0;j<allCodes.length();j++){
                ...
                if(thisCode.nodeType == "LABEL" ||
                   ...
                   thisCode.nodeType == "JNE"){
                    continue;
                }else if(thisCode.nodeType == "ASM"){
                    for(int k = 1;k<thisCode.args.length();k++){
    ...
    for(int i = 0;i<replseList.length();i++){
        ...
        if(nodeType=="ADD"||
           ...
           if(Atype=="DOUBLE" || Atype=="ULONG" || Atype=="LONG"){//64位运算
               ...
               QString va_name = A.useVar->varName;
               QString vb_name = argB_isConstValue ? QString() : B.useVar->varName;
               QString vc_name = argC_isConstValue ? QString() : C.useVar->varName;
               ...
               if(argC_isConstValue){
                ...
                code.args = QStringList({va_name,vb_text,vc_text});
                codeList += code;
           }
            ...
            if(Atype=="DOUBLE" || Atype=="ULONG" || Atype=="LONG"){//64位运算
                ...
                if(argC_isConstValue){
                 ...
                 retVarCode.args = QStringList({va_name,"FLAG"});
                 codeList += retVarCode;
            }
            ...
            if(mov_type==MOV ||
               ...
               mov_type==FTI){
                ...
                vb.isConstValue = argB_isConstValue;
                vb.operText = argB_isConstValue ? B.constValue : B.useVar->varName;
                vb.argIs64Bit = mov_type==QWORD_T_DWORD || mov_type == QWORD_T_QWORD;
                vb.constValueSize = B.size;
            ...
            if(Atype=="ULONG" || Atype=="LONG"){//64位运算
                ...
                if(argC_isConstValue){
                 ...
                 MiddleNode code;
                 code.nodeType = orderName;
                 code.args = QStringList({va_name,vc_text});
                 codeList += code;
            }
            ...
            if(Atype=="ULONG" || Atype=="LONG"){//64位运算
                ...
                code.nodeType = "MOV";
                code.args = QStringList({va_name,"FLAG"});
                codeList += code;
            ...
            QString va_name = A.useVar->varName;
            QString vb_name = (B.useVar == NULL) ? QString() : B.useVar->varName;

            if(getVarTypeSize(Atype)<=4){
                if(B.useVar!=NULL){//非立即数运算
                    tmp.nodeType = nodeType=="STORE" ? "WE" : "RD";
                    tmp.args = QStringList({rwSize,va_name,vb_name});
                    codeList.append(tmp);
                }else{//立即数运算
                    ...
                    //运算
                    tmp.nodeType = nodeType=="STORE" ? "WE" : "RD";
                    tmp.args = QStringList({rwSize,va_name,tempVar});
                    codeList.append(tmp);
                }
            }else{
                QString tempVar = getTempVar(varUseInfo);

                QStringList name = bit64VarTo32VarName(va_name);
                if(B.useVar!=NULL){//非立即数运算
                    tmp.nodeType = nodeType=="STORE" ? "WE" : "RD";
                    tmp.args = QStringList({rwSize,name[0],vb_name});
                    codeList.append(tmp);
                    tmp.nodeType = "UADD";
                    tmp.args = QStringList({tempVar,vb_name,"4d"});
                    codeList.append(tmp);
                    tmp.nodeType = nodeType=="STORE" ? "WE" : "RD";
            ...
            if(A.useVar!=NULL){
                ...
                if(getVarTypeSize(Atype)<=4){
                    ...
                    tmp.args = QStringList({rwSize,va_name});
                    codeList.append(tmp);
                }else{
                    tmp.nodeType = nodeType;
                    QStringList name = bit64VarTo32VarName(va_name);
            ...
            codeList.append(tmp);
        }else if(nodeType=="ASM" && thisCode.args.length()>=1){
            tmp.nodeType = thisCode.args[0].toUpper();
            for(int j = 1;j<thisCode.args.length();j++){
                tmp.args += thisCode.args[j];
            }
            codeList += tmp;
        }else{
            //不支持的运算
