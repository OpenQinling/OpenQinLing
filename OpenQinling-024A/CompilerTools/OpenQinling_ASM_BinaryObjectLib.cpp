...
QString BinaryMemBlock::toString(){
    ...
    QString isConstText = isCodeType==DataMem && isConstData ? "-常量" : "";
    QString isZIText = isCodeType==DataMem && isZIData ? "-非初始化" : "";
    QString typeText = isCodeType ? "指令型" : "数据型";
...
QByteArray BinaryObject::toFormatByteArray(){
    ...
    int marks_import_Len = marks_import.length();
    int blocks_Len = blocks.length();

    stream<<marks_import_Len;
    for(int i = 0;i<marks_import_Len;i++){
    ...
    for(int i = 0;i<blocks_Len;i++){
        ...
        stream<<thisBlock.isCodeType;
        stream<<thisBlock.binLength;
        stream<<thisBlock.bin;

        int marks_Len = thisBlock.marks.length();
        int markQuotes_Len = thisBlock.markQuotes.length();
        stream<<marks_Len;
        ...
        stream<<markQuotes_Len;
        for(int j = 0;j<markQuotes_Len;j++){
