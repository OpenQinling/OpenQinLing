...
bool judgeIsKeyword(QString txt){
    static const QStringList allKeyword ={
        ...
        "__valist__",
        //内联函数关键字(inline 函数定义) 暂未实现
        "inline",
        //禁止变量被优化关键字(一旦开启，该变量必然被存在内存中，不会临时存于堆栈，且每次取值都会从内存中，一旦修改立即回写内存) 暂未实现
        "volatile"
        //内嵌汇编关键字:   __asm__("汇编语句文本");  示例: __asm__("add c,a,b");
        "__asm__",
...
//替换字符串中的转义字符
QByteArray replaceTransChar(QString strData){//CharCodeSystem codeSystem
