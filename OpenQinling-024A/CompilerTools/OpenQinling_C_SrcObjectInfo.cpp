       ...
       (var.valueType == ConstValueType_POINTER || var.factIsPointer)){
        ...
        if(addressVar.factIsPointer){
            ...
            if(converType==ConstValueType_INT8){
                ...
                median = (uint32_t)median;
            }else if(converType==ConstValueType_UINT64 || converType==ConstValueType_POINTER){
                median = (uint64_t)median;
            }else{
                return tmp;
            }
    ...
    if((this->valueType == ConstValueType_POINTER || this->factIsPointer) ||
       (var.valueType == ConstValueType_POINTER || var.factIsPointer)){
        if(var.valueType == ConstValueType_POINTER || var.factIsPointer){
            return tmp;//被减数是指针,不符合语法要求
        }else if(var.valueType == ConstValueType_DOUBLE || var.valueType == ConstValueType_FLOAT){
            return tmp;//偏移地址是浮点数,不符合语法要求
        }
...
bool SrcAllDataTypeTree::appendNewTypeName(QString newName,QStringList bindTypeName,bool isDefaultConst,bool isDefaultPointerConst,int pointerLevel){
    ...
    info.isDefaultPointerConst = isDefaultPointerConst || info.isDefaultPointerConst;
...
bool SrcAllDataTypeTree::appendNewTypeName(QString newName,//类型名
    ...
    info.pointerLevel = pointerLevel;
    info.isDefaultPointerConst = isDefaultPointerConst;
    thisNodeTable.typeNameMapping.insert({newName},info);
    return 1;
}
...
//定义一个变量(返回定义成功后的变量索引号)
IndexVN SrcObjectInfo::defineVar(VarDefineInfo &varInfo){
    QString psdlName;
    if(this->getCurrentAnalysisType() == CurrentAnalysis_Global){
        psdlName = varInfo.varName;
    }else if(varInfo.isStatic){
        QString funName = allDefineFunction[allDefineFunction.length()-1].functionName;
        psdlName = "@"+funName+"_"+varInfo.varName+"_StaticVar"+QString::number(currentFuncVarCount);
        currentFuncVarCount++;
    }else{
        psdlName = varInfo.varName+"_"+QString::number(currentFuncVarCount);
        currentFuncVarCount++;
    }
    varInfo.psdlName = psdlName;
    return getVarDefineInfoNode()->defineVar(varInfo);
}

IndexVN SrcObjectInfo::defineValist(){
    if(getCurrentAnalysisType() != CurrentAnalysis_Function)return IndexVN();

    VarDefineInfo varInfo;
    varInfo.isConst = 0;
    varInfo.isExtern = 0;
    varInfo.isStatic = 0;
    varInfo.varName = "__valist__";
    varInfo.psdlName = "__valist__";
    varInfo.arraySize = {0};
    varInfo.pointerLevel = 0;
    varInfo.baseTypeIndex = this->getDataTypeInfoNode()->searchDataType({"void"});
    varInfo.isFuncVar = 1;
    return getVarDefineInfoNode()->defineVar(varInfo);
}

//添加一个宏定义
bool SrcObjectInfo::defineMacro(QString name,DefineMacroInfo value){
    if(defines.contains(name))return 0;
    defines.insert(name,value);
    return 1;
}
//取消一个宏定义
void SrcObjectInfo::undefineMacro(QString name){
    defines.remove(name);
}
//获取一个宏定义的信息
DefineMacroInfo SrcObjectInfo::getMacroValue(QString name){
    if(!defines.contains(name))return DefineMacroInfo();
    return defines[name];
}
//是否存在宏
bool SrcObjectInfo::containsMacro(QString name){
...
//创建一个匿名标记点
QString SrcObjectInfo::defineAnonyMark(){
    return "@anonyMark"+QString::number(currentAnonyMarkCount++);
}

//在函数中添加一个独立的临时变量[生成一个独立的psdl名称,并在psdl的vars列表中添加该变量的定义](返回成功后的变量索引号)
//baseType:临时变量的基础数据类型,pointerLevel:临时变量的指针级数 (临时变量不可能是一个数组)
IndexVN SrcObjectInfo::defineFuncTmpVar(IndexVN baseType,
                         int pointerLevel,
                         QList<PsdlIR_Compiler::MiddleNode> &varsAtt){
    return defineFuncTmpVar(baseType,pointerLevel,QList<int>(),varsAtt);
}
IndexVN SrcObjectInfo::defineFuncTmpVar(IndexVN baseType,
                         int pointerLevel,
                         QList<int>arraySize,
                         QList<PsdlIR_Compiler::MiddleNode> &varsAtt){
    QString tmpVarName = "_Tmp"+QString::number(currentFuncTmpVarCount)+"_";
    VarDefineInfo varInfo;
    varInfo.baseTypeIndex = baseType;
    varInfo.pointerLevel = pointerLevel;
    varInfo.arraySize = arraySize;
    varInfo.varName = tmpVarName;
    varInfo.psdlName = tmpVarName;
    varInfo.isFuncVar = 1;
    varInfo.isConst = 0;
    currentFuncTmpVarCount++;

    DataTypeInfo * typeInfo = this->getDataTypeInfoNode()->getDataType(baseType);
    DataBaseType dataBaseType = typeInfo->baseType;

    PsdlIR_Compiler::MiddleNode psdlVars;
    QString typeName;
    bool isArray = 0;
    if(pointerLevel > 0 || dataBaseType==DataBaseType_FUNPTR){
        typeName = "POINTER";
    }else if(dataBaseType == DataBaseType_INT){
        typeName = "INT";
    }else if(dataBaseType == DataBaseType_UINT){
        typeName = "UINT";
    }else if(dataBaseType == DataBaseType_SHORT){
        typeName = "SHORT";
    }else if(dataBaseType == DataBaseType_USHORT){
        typeName = "USHORT";
    }else if(dataBaseType == DataBaseType_CHAR){
        typeName = "BYTE";
    }else if(dataBaseType == DataBaseType_UCHAR){
        typeName = "UBYTE";
    }else if(dataBaseType == DataBaseType_LONG){
        typeName = "LONG";
    }else if(dataBaseType == DataBaseType_ULONG){
        typeName = "ULONG";
    }else if(dataBaseType == DataBaseType_FLOAT){
        typeName = "FLOAT";
    }else if(dataBaseType == DataBaseType_DOUBLE){
        typeName = "DOUBLE";
    }else if(dataBaseType == DataBaseType_UNION ||
               dataBaseType == DataBaseType_STRUCT){
        isArray = 1;
    }
    if(isArray){
        psdlVars.nodeType = "ARRAY";
        psdlVars.args = QStringList({tmpVarName,QString::number(typeInfo->dataBaseTypeBytes)+"D"});

    }else{
        psdlVars.nodeType = "DATA";
        psdlVars.args = QStringList({typeName,tmpVarName});
    }
    varsAtt.append(psdlVars);
    return getVarDefineInfoNode()->defineVar(varInfo);
}

//在函数中添加一个映射到另一变量的临时变量[psdl名称使用已存在的另一个变量的名称](返回成功后的变量索引号)
//baseType:临时变量的基础数据类型,pointerLevel:临时变量的指针级数 (临时变量不可能是一个数组)
IndexVN SrcObjectInfo::defineFuncLinkVar(IndexVN baseType,int pointerLevel,QString psdlName){
    QString tmpVarName = "_Link"+QString::number(currentFuncLinkVarCount)+"_";
    VarDefineInfo varInfo;
    varInfo.baseTypeIndex = baseType;
    varInfo.pointerLevel = pointerLevel;
    varInfo.varName = tmpVarName;
...
QString SrcObjectInfo::appendAnonyConstStrValue(QString strValue,int&strByteSize){
    ...
    QByteArray stringBytes = replaceTransChar(strValue);
