...
//将中级语言节点结构信息转为文本表示
QString MiddleNode::toString(bool isRoot){
    ...
    foreach(MiddleNode_Att att,atts){
        additionalArgText+="."+att.attName.toUpper();
...
MiddleNode::MiddleNode(QString &src,bool &suceess,bool level){
...
bool MiddleNode::areThereOnlyTheseAtt(QStringList attNams){
...
MiddleNode_Att* MiddleNode::getAttFromName(QString name){
    for(int i = 0;i<atts.length();i++){
...
QString MiddleNode::segmentStringThroughText(QString &src,QString text,QStringList stopTexts,bool &suceess){
    QStringList list;
    QString ret;
    list = src.split(text);
...
QString MiddleNode::extractParentContent(QString &src,bool &suceess){
    int level = 1;
    suceess = 0;
...
QString MiddleNode::extractString(QString txt,QStringList* bank,bool*isWindup,int*strStartp){
...
//从标志号中提取出字符串
QString MiddleNode::getStringFromFlag(QString flagTXT,QString typeCode,QStringList &str,bool*isSrceess){
...
void MiddleNode::replaceNodeStr(MiddleNode &node,QStringList&strBlock){
