...
namespace Driver{
//判断是否已经存在指定的命令
bool TerminalCmd::cmdOrderIsExist(QString order,QList<TerminalCmd> &terCmds){
...
//根据命令名获取命令的索引
TerminalCmd& TerminalCmd::getTerminalCmd(QString order,QList<TerminalCmd> &terCmds){
    for(int i = 0;i<terCmds.length();i++){
        if(terCmds[i].order==order){
            return terCmds[i];
        }
...
bool TerminalCmd::isOnlyHaveTheseOrder(QStringList orders,QList<TerminalCmd> &terCmds){
    for(int i = 0;i<terCmds.length();i++){
...
QList<TerminalCmd> TerminalCmd::parsingTerminalCmd(QStringList cmds){
