...
BinaryObject generateBinaryObj(QList<AsmSrc> &src){
    ...
    for(int k = 0;k<src.length();k++){
        ...
        for(int i = 0;i<thisSrc.memBlocks.length();i++){
            ...
            block.isCodeType = thisSrc.memBlocks[i].type;
...
BinaryObject complieAsmSrc(QString srcText,QString srcPath,bool * isSuceess,QStringList*prompts){
    ...
    QList<AsmSrc> asmSrcs({SrcGrammaticalAnalysis(srcText,srcPath,prompts)});
    if(asmSrcs[0].iseffective==false){
        //解析源文件全局指令失败，退出编译器程序
        if(isSuceess!=NULL)*isSuceess=0;
        return BinaryObject();
    }
    bool isHaveBlockRepeat = detectDuplicateNames(asmSrcs,prompts);//检测所有定义的内存块是否有重复的名称
    detectDuplicateImport(asmSrcs,prompts);//去除重复引入的
    ...
    bool analysisBlockSuceess = analysisAsmBlockCode(asmSrcs[0],prompts);
    ...
    bool isMarkBlockRepeat = detectDuplicateDefMark(asmSrcs[0],prompts);
    ...
    //提取出当前所有源文件定义或引入的标记
    QStringList allImportMarks = extractAllImportLabel(asmSrcs);//所有引入的标记
    QStringList allDefineMarks = extractAllDefineLabel(asmSrcs);//所有定义的标记

    //编译数据内存块
    bool dataBlockIsSuceess = compileDataMemBlock(asmSrcs[0],allImportMarks,allDefineMarks,prompts);

    //编译指令内存块
    bool codeBlockIsSuceess = compileCodeMemBlock(asmSrcs[0],allImportMarks,allDefineMarks,prompts);
    ...
    return generateBinaryObj(asmSrcs);
}
