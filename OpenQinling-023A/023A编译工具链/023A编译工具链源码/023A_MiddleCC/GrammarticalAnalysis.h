...
QString analysisGlobalVariable(MiddleNode &node,QStringList&allAddressLabel,bool&status){
    ...
    if(node.areThereOnlyTheseAtt({"CONST","EXPORT","BODY"})){
        debugNodeLine("全局变量定义了不支持的属性",node);
        return asmText;
    }
    ...
    if(node.nodeType=="VAR"){
        ...
        if(!varTypeIsConform(type)){
            debugNodeLine("全局变量类型定义不正确",node);
            return asmText;
        }
        QString &name = node.args[1];
        if(!nameIsConform(name)){
            debugNodeLine("全局变量名称定义不正确",node);
            return asmText;
        }
        asmText = "DATA";
        ...
        if(node.args.length()==2){
            ...
            if(suceess==0){
                debugNodeLine("全局变量初始值定义不正确",node);
                return asmText;
            }
        ...
        if(node.atts.length()!=0 || node.args.length()!=1){
            debugNodeLine("外部标记定义语法不正确",node);
            return asmText;
        }

        QString &name = node.args[0];
        if(!nameIsConform(name)){
            debugNodeLine("外部标记名称定义不正确",node);
            return asmText;
        }
        ...
        if(node.getAttFromName("BODY")==NULL || node.args.length()!=1){
            debugNodeLine("全局变量组定义语法不正确",node);
            return asmText;
        }

        QString &group_name = node.args[0];
        if(!nameIsConform(group_name)){
            debugNodeLine("全局变量组名称定义不正确",node);
            return asmText;
        }
        ...
        for(int i = 0;i<bodyNode->subNodes.length();i++){
            ...
            if(varNode.atts.length()!=0){
                debugNodeLine("变量组属性定义语法不正确",varNode);
                return asmText;
            }
            if(varNode.nodeType=="VAR"&&(varNode.args.length()==2||varNode.args.length()==3)){
                ...
                if(varNode.args.length()==2){
                    ...
                    if(suceess==0){
                        debugNodeLine("全局变量组属性初始值定义不正确",varNode);
                        return asmText;
                    }
                    if((valueType=="DOUBLE" || valueType=="FLOAT")&&(type!="DOUBLE" && type!="FLOAT")){
                        debugNodeLine("全局变量组属性初始值定义不正确",varNode);
                ...
                if(suceess==0){
                    debugNodeLine("变量组属性数组长度定义不正确",varNode);
                    return asmText;
                }
        ...
        if(node.args.length()==0){
            debugNodeLine("全局数组定义语法不正确",node);
            return asmText;
        }
        QString &name = node.args[0];
        if(!nameIsConform(name)){
            debugNodeLine("全局数组名称定义不正确",node);
            return asmText;
        }
        MiddleNode_Att * bodyNode = node.getAttFromName("BODY");
        asmText = "DATA";
        ...
        if(bodyNode==NULL && node.args.length()==2){
            ...
            if(suceess==0){
                debugNodeLine("全局数组长度定义不正确",node);
                return asmText;
            }
            if(valueType=="DOUBLE" || valueType=="FLOAT"||valueText.at(0)=='-'){
                debugNodeLine("全局数组长度定义不正确",node);
            ...
            for(int i = 0;i<bodyNode->subNodes.length();i++){
                MiddleNode &varNode = bodyNode->subNodes[i];
                if(varNode.atts.length()!=0){
                    debugNodeLine("全局数组初始值定义语法不正确",varNode);
                    return asmText;
                }
                if(varNode.nodeType=="VAR"&&(varNode.args.length()==2||varNode.args.length()==1)){
                    ...
                    if(!varTypeIsConform(type)){
                        debugNodeLine("全局数组初始值类型定义不正确",varNode);
                        return asmText;
                    }
                    if(varNode.args.length()==2){
                        notInit = 0;
                        ...
                        QString valueText = analysisConstValue(varNode.args[1],allAddressLabel,suceess,valueType,valueSize);
                        if(suceess==0){
                            debugNodeLine("全局数组初始值定义不正确",varNode);
                            return asmText;
                        }
                        if((valueType=="DOUBLE" || valueType=="FLOAT")&&(type!="DOUBLE" && type!="FLOAT")){
                            debugNodeLine("全局数组初始值定义不正确",varNode);
                    ...
                    if(suceess==0){
                        debugNodeLine("全局数组不初始化部分定义语法不正确",varNode);
                        return asmText;
                    }
                    if(valueType=="DOUBLE" || valueType=="FLOAT"||valueText.at(0)=='-'){
                         debugNodeLine("全局数组不初始化部分定义语法不正确",varNode);
                    ...
                    if(varNode.args[0].at(0)!='\"'){
                        debugNodeLine("全局数组字符串定义语法不正确",varNode);
                        return asmText;
                    }
                    varAsmText += "\tSTR "+varNode.args[0]+";\r\n";
                }else{
                     debugNodeLine("全局数组初始化定义语法不正确",varNode);
                    return asmText;
                }
        ...
        debugNodeLine("全局变量定义语法不正确",node);
        return asmText;
    }
    status = 1;
    return asmText;
...
QString analysisAsmFun(MiddleNode &node,QStringList&allAddressLabel,bool&status){
    ...
    if(node.getAttFromName("BODY")==NULL || node.args.length()!=1){
        ...
        debugNodeLine("定义汇编函数语法有误",node);
        return asmText;
    }

    QString &name = node.args[0];
    if(!nameIsConform(name)){
        debugNodeLine("汇编函数名称定义有误",node);
        return asmText;
    }
    bool isExport = node.getAttFromName("EXPORT");
