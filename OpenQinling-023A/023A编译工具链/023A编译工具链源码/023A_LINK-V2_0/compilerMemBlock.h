#ifndef COMPILERMEMBLOCK_H
#define COMPILERMEMBLOCK_H
...
//检查内存块的定义是否符合要求
bool checkMemBlockDef(QList<LibraryFile> &files,LinkSprict &link,QStringList *prompts){
    QStringList public_codeBlocksName,public_dataBlocksName;
    QStringList public_markDef;
    foreach(LibraryFile lib,files){
        foreach(BinaryObject obj,lib.libData){
            foreach(BinaryMemBlock block,obj.blocks){
                if(block.isExport){
                    if(block.type==CodeMem){
                        public_codeBlocksName.append(block.name);
                    }else if(block.type==DataMem){
                        public_dataBlocksName.append(block.name);
                    }
                    foreach(BinaryMark mark,block.marks){
    ...
    bool isHave = 0;
    foreach(QString checkCode,link.checkCode){
        if(!public_codeBlocksName.contains(checkCode)){
            prompts->append("\033[31m错误:\033[0m存在没有定义的指令内存块<"+checkCode+">");
            isHave = 1;
        }
    }
    foreach(QString checkData,link.checkData){
        if(!public_dataBlocksName.contains(checkData)){
            prompts->append("\033[31m错误:\033[0m存在没有定义的数据内存块<"+checkData+">");
            isHave = 1;
        }
    }
    if(isHave){
    ...
    foreach(LibraryFile lib,files){
        foreach(BinaryObject obj,lib.libData){
            foreach(BinaryImportMark mark,obj.marks_import){
...
QStringList getMemBlockUseBlock(BinaryMemBlock &mem){
    QStringList names;
    foreach(BinaryQuoteMark quote,mem.markQuotes){
        names.append(quote.affiBlockName);
    }
    return names;
}
//判断一个内存块是否属于指定的obj中的私有内存块
bool thisBlockIsPrivateFromObj(QString blockName,BinaryObject & obj){
    foreach(BinaryMemBlock mem,obj.blocks){
...
BinaryMemBlock* getPublicMem(QList<LibraryFile> &files,QString name,BinaryObject*fromObj=NULL){
    for(int i = 0;i<files.length();i++){
        LibraryFile &file = files[i];
        for(int j = 0;j<file.libData.length();j++){
            BinaryObject &obj = file.libData[j];
...
//在指定的obj中搜索内存块
BinaryMemBlock* getMemFromObj(BinaryObject & obj,QString name){
    for(int k = 0;k<obj.blocks.length();k++){
...
void getAllUsePublicMem(QList<LibraryFile> &files,QStringList root,QStringList*all){
    foreach(QString name,root){
        BinaryObject obj;

        BinaryMemBlock * block=NULL;
...
//获取一个obj中所有使用的内存块的名字
void getAllUseMem(BinaryObject & obj,QStringList root,QStringList*all){
    foreach(QString name,root){
        BinaryMemBlock *block = getMemFromObj(obj,name);
        if(block==NULL){
            continue;
        }
        QStringList tmps = getMemBlockUseBlock(*block);
...
//过滤掉各个obj中无用的私有内存块
void removMemBlockInObj(BinaryObject & obj){
...
QStringList getIndirectUsePublicMen(QList<LibraryFile> &files,QStringList checkMemNames){
    ...
    foreach(QString checkMemName,checkMemNames){
        ...
        foreach(QString thisBlockName,thisObjUseMemNames){
            BinaryMemBlock *thisBlock = getMemFromObj(obj,thisBlockName);
...
void filterMemBlock(QList<LibraryFile> &files,LinkSprict &link){
...
bool tryAllocMem(BinaryMemBlock &block,QList<BlockInfo> &memSpace,bool isAppoint = 0,uint appointAdd = 0){
    if(isAppoint){
        BlockInfo info;
        info.baseAdress = appointAdd;
        info.sizeBytes = block.binLength;
        if(!judgeContainBlockSet(memSpace,{info})){
            return 0;
        }
        memSpace = removeOccupiedMemBlock(memSpace,info);
        block.baseAdd = appointAdd;
    }else{
        bool isScueess;
        BlockInfo info = searchFreeMemarySpace(memSpace,block.binLength,&isScueess);
        if(!isScueess){
            return 0;
        }
        memSpace = removeOccupiedMemBlock(memSpace,info);
        block.baseAdd = info.baseAdress;
    }
    return 1;
}

//根据名字获取内存块中定义的一个标记的对象
BinaryMark * getMarkFromMem(BinaryMemBlock &mem,QString markName){
...
void setBitDataFromByteArray(QByteArray & byteArray,uint offset_Byte,uint offset_Bit,
                             uint value,uint higth,uint low){
    ...
    uchar tmp2[4];
        ...
        memreverse(tmp2,tmp,4);
    #else
        memcpy(tmp2,tmp,4);
    #endif
    QByteArray valueBytes(4,0);
    for(int i = 0;i<4;i++){
        valueBytes[i] = tmp2[i];
    }
...
ExecutableImage compilerMemBlock(QList<LibraryFile> &files,LinkSprict &link,bool *isSuceess,QStringList *prompts){
    ...
    QList<BinaryMemBlock*> fixedMemPtrList;//用于后续为所有内存块分配基地址时,排除掉由FIXED命令指定基地址的内存块
    foreach(QString fixedBlockName,link.fixed.keys()){
        BinaryMemBlock* block = getPublicMem(files,fixedBlockName);
        if(block!=NULL){
    ...
    for(int i = 0;i<files.length();i++){
        ...
        for(int j = 0;j<file.libData.length();j++){
            BinaryObject &obj = file.libData[j];
            for(int k = 0;k<obj.blocks.length();k++){
                BinaryMemBlock *thisBlock = &obj.blocks[k];
    ...
    for(int i = 0;i<files.length();i++){
        ...
        for(int j = 0;j<file.libData.length();j++){
            BinaryObject &obj = file.libData[j];
            for(int k = 0;k<obj.blocks.length();k++){
                BinaryMemBlock &block = obj.blocks[k];
                if(block.isZIData && block.type==DataMem){
                ...
                for(int z = 0;z<block.markQuotes.length();z++){
                    BinaryQuoteMark &quote =block.markQuotes[z];
                    //解析内存块中引用的标记指向的绝对地址
                    BinaryMemBlock * markFromBlock;
                    if(quote.isImport){
                    ...
                    BinaryMark * mark = getMarkFromMem(*markFromBlock,quote.name);
