...
static QList<FunArgDefInfo> analysisFunctionArgDef(PhraseList &sentence,
    ...
    for(int i = 0;i<allArgSence.length();i++){
        ...
        if(allArgSence[i].getKeyWordNum("const")){
            argIsConst = 1;
            allArgSence[i].removeKeyWord("const");
        }
        ...
        if(isStructEnumUnionFun==4){
            ...
            bool isHaveName;
            QList<int> arraySize;
            attTypeIndex = analysisFunctionPtrTypeDef(allArgSence[i],objInfo,name,isHaveName,argPointerLevel,arraySize);
            argPointerLevel+=arraySize.length();

            if(attTypeIndex.isEmpty())return QList<FunArgDefInfo>();
        }else if(isStructEnumUnionFun){
    ...
    for(int i = 0;i<defineVarInfos.length();i++){
        if(defineVarInfos[i].type==DefineFun){
            ...
            if(structPtr->baseType==DataBaseType_UNION){
                ...
                att.offset = structPtr->dataBaseTypeBytes;
            }
            att.isConst = isConst;
            ...
            DataTypeInfo *attTypePtr = objInfo.getRootDataTypeInfoNode()->getDataType(funPtrTypeIndex);
            ...
            uint attSize = attTypePtr->dataBaseTypeBytes;
            if(att.pointerLevel>0){
                attSize = objInfo.CPU_BIT/8;
            }
            ...
            if(structPtr->baseType==DataBaseType_UNION){
                if(structPtr->dataBaseTypeBytes<attSize){
                    structPtr->dataBaseTypeBytes = attSize;
                }
            }else{
                structPtr->dataBaseTypeBytes += attSize;
            }
            ...
            bool status;
            bool isAutoArrSize;
            QString name = analysisDefVarOrAtt_Name_Ptr_Arr(defineVarInfos[i].pls,objInfo,var_pointerLevel,var_array_size,isHaveName,isAutoArrSize,status);
            ...
            att.pointerLevel = var_pointerLevel;
            att.typeIndex = typeIndex;
            att.attIndex = attIndex;

            DataTypeInfo *attTypePtr = objInfo.getRootDataTypeInfoNode()->getDataType(att.typeIndex);
...
static IndexVN analysisStructUnionTypeDef(PhraseList &sentence,SrcObjectInfo &objInfo){
    ...
    for(int i = 0;i<attDefineS.length();i++){
        ...
        if(attDefineS[i].getKeyWordNum("typedef")){
            ...
            //定义一个属性
            if(analysisStructUnionType_AttDef(i,index,attDefineS[i],objInfo)==0){
                return IndexVN();
            }
        }
    }
    ...
    for(int i = 0;i<defineVarInfos.length();i++){
        ...
        if(defineVarInfos[i].type==DefineFun){
            ...
            if(!objInfo.getDataTypeInfoNode()->appendNewTypeName(name,//类型名
                                       typeIndex,//绑定到的表
                                       index,//表中的哪个类型
                                       isConst,var_pointerLevel + pointerLevel)){
                isHaveError = 1;
                objInfo.appendPrompt("该数据类型已存在,不可重复定义",thisSrcPath,thisLine,thisCol);
                continue;
             }
...
static QList<InitValuePlace> analysisGlobalVarInitValuePlace(IndexVN typeIndex,
    ...
    if(arrSize.length()==1 &&
        ...
        place.isStringArray = replaceTransChar(sentence[0].text,objInfo.cstringCodeSystem).length();//
...
static bool analysisGlobalVarInitValue(IndexVN varIndex,
    ...
    while(places.length()!=0){
        ...
        if(thisValuePlace.isStringArray){
            //存储常量字符串的字符数组
            QByteArray strByte = replaceTransChar(thisValuePlace.ext[0].text,objInfo.cstringCodeSystem);
...
static QList<PsdlIR_Compiler::MiddleNode_Att> analysisExtendAttribute(PhraseList &sentence,
    ...
    PhraseList replace;
    for(int i = 0;i<sentence.length();i++){
        if(sentence[i].PhraseType == Phrase::ExtendKeyWord){
            PhraseList keyWordDefSentence = sentence[i].subPhraseList;
            ...
            foreach(PhraseList keyWord,keyWordList){
                ...
                if(!attNames.contains(keyWord[0].text)){
                    attNames.append(keyWord[0].text);
                }
            }
        }else{
...
struct FuncOperValue{
    ...
    FuncOperValue dereference(){
        ...
        if(funcOperValueType == FuncOperValueType_Function){
            ...
            if(info.pointerLevel==0 && info.arraySize.length()==0)return value;

            if(info.arraySize.length() != 0){
                value.varIndex = objInfo->defineFuncLinkVar(voidType,1,info.varPsdlName);
                value.quote_IsConst = info.isConst;//引用是否是常量取决于数组是否是常量
    ...
    FuncOperValue getPointerStructAtt(QString attName){
        ...
        value.quoteOrConst_Type = att.typeIndex;
        value.quote_ArraySize = att.arraySize;
        value.quote_IsConst = ptrIsConst;//解得的属性引用是否是常量取决于指针是否是常量
    ...
    FuncOperValue assignment(FuncOperValue &assignValue){
        ...
        if(assignValue.funcOperValueType==FuncOperValueType_Function){
            ...
            RegDataInfo valueInfo;
            this->getInfo_RegData(valueInfo);
            if(valueInfo.isDummyVar || (!valueIsPointer && valueInfo.isConst)){
                return retValue;
            }
    ...
    static FuncOperValue generateQuaternionOper(FuncOperValue v1,FuncOperValue v2,QString typeName){
        ...
        if(v1.funcOperValueType == FuncOperValueType_ConstValue && v2.funcOperValueType==FuncOperValueType_ConstValue){
            ...
            if(typeName=="ADD")constVarRet = constVar1+constVar2;
            else if(typeName=="SUB")constVarRet = constVar1-constVar2;
        ...
        if(isPointerOper){//指针+-运算
            ...
            if(isUseFunc2){
                ...
                processCode.args = QStringList({typeName=="BOR" ? "EC" : "NEC",
                                                resultVarName,
                                                "<UBYTE:0D>"});
                PsdlIR_Compiler::MiddleNode_Att judgeAtt;
                judgeAtt.attName = "JUDGE";
                judgeAtt.subNodes = v1.bodyAtt;
                judgeAtt.subNodes += v1MovCode;
                PsdlIR_Compiler::MiddleNode_Att ifAtt;
                ifAtt.attName = "IF";
                ifAtt.subNodes = v2.bodyAtt;
                ifAtt.subNodes += v2MovCode;
                processCode.atts.append(judgeAtt);
    ...
    static FuncOperValue ifelseOper(FuncOperValue judge,FuncOperValue ifValue,FuncOperValue elseValue){
        ...
        if(judge.funcOperValueType == FuncOperValueType_RegData ||
           judge.funcOperValueType == FuncOperValueType_QuoteData){
            ...
            if(judge.bodyAtt.length()!=0){
                PsdlIR_Compiler::MiddleNode_Att judgeCode;
                judgeCode.attName = "JUDGE";
                judgeCode.subNodes.append(judge.bodyAtt);
                processCode.atts.append(judgeCode);
            }
    ...
    for(int i = 0;i<sentence.length();i++){
        ...
        if(sentence[i].isKeyWord("sizeof") && i+1<sentence.length() &&
           sentence[i+1].isDelimiter("()")){
            ...
            //字符常量 转为uint64的常量
            bool su;
            long long cv = analysisCharConstValue(sentence[i].text,su);
            FuncOperValue value(cv,varsAtt,objInfo);
            tmp.append(value);
            converIsValue = 1;
        }else if(sentence[i].PhraseType == Phrase::ConstValue_STRING){
            //字符串常量 转为一个char[]的引用
...
static bool analysisFunDefineVarInitValueOrder(IndexVN varIndex,//传入要初始化的变量索引号
    ...
    while(places.length()!=0){
        ...
        if(thisValuePlace.isStringArray){
            ...
            //存储常量字符串的字符数组
            QByteArray strByte = replaceTransChar(thisValuePlace.ext[0].text,objInfo.cstringCodeSystem);//字符串的初始值
            ...
            if(strByte.length() > 10){//如果字符数>20个，创建一个静态字符串常量数组，通过ARRCOPY指令拷贝到数组中
                ...
                getPtrAddress.args = QStringList({tmpVar2Name,varInfo.psdlName,QString::number(unitOffset)+"D"});
            ...
            PsdlIR_Compiler::MiddleNode getPtrAddress;
            getPtrAddress.nodeType = "ADDRESS";
            getPtrAddress.args = QStringList({tmpVarName,varInfo.psdlName,QString::number(unitOffset)+"D"});
    ...
    while(sentence.length()!=0){
        ...
        if(sentence.length()>=3 &&
           ...
           sentence[1].isDelimiter("()")){//if-else选择跳转语句
            ...
            operUsePhraseCount = 1;
        }else if(sentence.getDelimiterNum(";")){//变量定义或运算语句
            //解析是否是定义变量语句
            bool isDefineVar;
...
//解析内嵌汇编函数
//内嵌汇编函数不支持void返回值,参数只支持void*,不支持stdcall
static bool analysisDefineAsmFunction(PhraseList &sentence,
                            SrcObjectInfo &objInfo){
...
PsdlIR_Compiler::MiddleNode analysisGrammatical_C(SrcObjectInfo &objInfo,
   ...
   while(phraseList.length()!=0){
       ...
       if(thisSentenseType==TypeDefine){
           ...
           bool isAsmFunc = sentence.judgeIsHaveAnyKeyWord({"__asm__"});
           if(isAsmFunc){
               //定义一个函数
               ...
               if(analysisDefineAsmFunction(sentence,objInfo)==0){
                   isHaveError = 1;
                   continue;
               }
           }
