...
static QList<SrcToken> getToken(SrcObjectInfo &srcObjInfo,
    ...
    for(int i = 0;i<tokens.length();i++){
        ...
        if(((txt.contains('F')||txt.contains('E'))&&!txt.contains('X')) ||txt.contains('.')){
            ...
            bool su;
            txt = QString::number(txt.toULongLong(&su,numSys));
            if(isUnsigned){
                txt += "U";
            }
            if(isLongLong){
...
PhraseList lexicalAnalyzer_C(SrcObjectInfo &srcObjInfo,
    ...
    //合并Phrase中的特殊扩展关键字
    for(int i = 0;i<srcExpList.length();i++){
        if(srcExpList[i].isKeyWord("__attribute__") && i+1<srcExpList.length() && srcExpList[i+1].isDelimiter("()")){
            srcExpList[i].PhraseType = Phrase::ExtendKeyWord;
            srcExpList[i].subPhraseList = srcExpList[i+1].subPhraseList;
            srcExpList.removeAt(i+1);
        }
    }
    return srcExpList;
}
