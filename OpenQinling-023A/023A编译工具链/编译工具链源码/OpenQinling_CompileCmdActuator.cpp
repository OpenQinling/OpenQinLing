...
const static QString helpPromptText = "\033[32m编译器支持的功能性命令(每次使用编译器都只可其中一种)\033[0m\r\n"
                                      ...
                                      "   -info  打印输出.lib或.etb文件中符号表、内存分配状况等信息(参数1个,传入待获取信息的文件路径)\r\n"
                                      "   (不传入任何命令，就表示将代码、静态库链接为可执行程序)\r\n"
                                      "\r\n"
...
//版本信息提示文本
const static QString versionPromptText = "\033[32mOpenQinling编译工具链:\033[0m\r\n"
                                   "适用CPU型号:  OpenQinling-023A      \r\n"
                                   "版本:         3.0.0(测试版,暂不支持C语言标准库)\r\n"
                                   "支持语言:     C语言(c99标准)\r\n"
                                   "              PSDL-IR中间表示语言\r\n"
                                   "              ASM汇编语言\r\n"
...
static int linkToExecFile(QStringList srcPaths,QString outPath,
    ...
    if(QFileInfo(outPath).completeSuffix().toUpper() != "ETB"){
    ...
    QByteArray bin = image.toEtbFileData();
    for(int i = 0;i<promptText.length();i++){
        qDebug()<<promptText[i].toUtf8().data();
    }
    if(!suceess){
        QString promptText = redColorTextBegin + "出错:链接为ETB文件失败"+colorTextEnd;
        qDebug()<<promptText.toUtf8().data();
        return -1;
    }
...
int compileCmdActuator(QString actuatorExeDir,
    ...
    //处理-info指令
    {
        if((Driver::TerminalCmd::cmdOrderIsExist("-info",cmds))){
            ...
            if(suffix != "LIB" &&suffix != "ETB"){
                QString promptText = redColorTextBegin + "出错:根据文件后缀名判断其非静态库文件和ETB文件["+cmd.args[0]+"]"+colorTextEnd;
                qDebug()<<promptText.toUtf8().data();
                return -1;
            }
