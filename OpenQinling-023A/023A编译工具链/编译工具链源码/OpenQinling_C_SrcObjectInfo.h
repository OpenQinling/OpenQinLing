...
class DataTypeInfo;
class SrcAllDataTypeTree;
//结构体/共用体属性信息、函数返回值/参数信息
class StructUnion_AttInfo{
public:
...
public:
    ...
    bool isDefaultConst = 0;//该类型是默认为一个常量
...
public:
 ...
 //////////////////增加类型定义//////////////////
    ...
    bool appendNewTypeName(QString newName,QStringList bindTypeName,bool isDefaultConst = 0,int pointerLevel = 0);
    //在当前节点添加一个自定义数据类型,通过索引号的方式
    bool appendNewTypeName(QString newName,//类型名
                           IndexVN tableIndex,//绑定到的表
                           int typeIndex,//表中的哪个类型
                           bool isDefaultConst = 0,int pointerLevel = 0);
...
struct SrcObjectInfo{
    ...
    CharCodeSystem cstringCodeSystem;//c语言中字符串的编码格式
    ...
    int anonyStrValueCount = 0;//全局匿名字符串常量的数量
    //全局匿名字符串常量转为psdl名称为 @AnonyStrValue{anonyStrValueCount}
