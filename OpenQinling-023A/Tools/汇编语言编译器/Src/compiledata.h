...
QByteArray compileData(QStringList asm_data,uint* isSuccess){
    ...
    foreach(QString order_txt,asm_data){
        ...
        if(order=="BYTE"||order=="byte"){
        ...
        }else if(order=="ARR"||order=="arr"){
            //数组(默认初始值0xCC)
            QStringList args = orderArgs.at(1).split(",");
            if(orderArgs.length()!=2){
                if(isSuccess!=NULL){
                    *isSuccess=0;
                }
                return compileError(index,order_txt,"args number error");
            }
            uint size_isSrc,len_isSrc;

            uint size = compileUInt(args.at(0),&size_isSrc);
            uint len = compileUInt(args.at(1),&size_isSrc);
            ...
            char *arr = new char[size*len];
            for(uint i = 0;i<size*len;i++){
                arr[i] = 0xcc;
            }
            bin.append(arr,size*len);
            delete [] arr;
        }
        else{
            if(isSuccess!=NULL){
