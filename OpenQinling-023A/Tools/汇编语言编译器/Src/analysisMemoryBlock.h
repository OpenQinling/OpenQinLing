        ...
        if(list[i].isMark){
            ...
            if(list[i].order=="INT"||list[i].order=="int"||
               ...
               offset+=4*argLength;
                ...
                offset+=argLength;
            }else if(list[i].order=="USHORT"||list[i].order=="ushort"||
                     list[i].order=="SHORT"||list[i].order=="short"){
                offset+=2*argLength;
            }else if(list[i].order=="ULONG"||list[i].order=="ulong"||
                     list[i].order=="LONG"||list[i].order=="long"||
                     list[i].order=="DOUBLE"||list[i].order=="double"){
                offset+=8*argLength;
            }else if(list[i].order=="ARR"||list[i].order=="arr"){
                offset+=compileUInt(list[i].args.at(0),NULL)*compileUInt(list[i].args.at(1),NULL);
        ...
        foreach(QString arg,args){
            if(arg.at(0)=="["&&( arg.at(arg.length()-1)=="]" || arg.at(arg.length()-1)==">" )){
                ...
                if(sectTXT_high==""&&sectTXT_low==""){
                    ...
                    //如果未确定位限制，则使用该指令的默认位限制
                    if(orderList[i].order=="MOV"||orderList[i].order=="mov"){

                        //转移指令，如果写入寄存器是ds,位限制默认<31-16>
                        if(args.at(0)=="DS"||args.at(0)=="ds"){
                            high = 31;
                            low = 16;
                        }
                        //如果写入寄存器是cs,位限制默认是<31-27>
                        else if(args.at(0)=="CS"||args.at(0)=="cs"){
                            high = 31;
                            low = 27;
                        }
                    }else if(orderList[i].order=="JMP"||orderList[i].order=="jmp"||
                             orderList[i].order=="JT"||orderList[i].order=="jt"){
                        //跳转指令，位限制默认<26-0>
                        high = 26;
                        low = 0;
                    }else if(orderList[i].order=="RD"||orderList[i].order=="rd"||
                             orderList[i].order=="WE"||orderList[i].order=="we"){
                        //内存读写指令，位限制默认<15-0>
                        high = 15;
                        low = 0;
                    }
...
uint getDataBlockSize(QList<Order>& orderList,QStringList& strTxt){
    ...
    foreach(Order o,orderList){
        if(o.order=="UINT"||o.order=="uint"||
        ...
        }else if(o.order=="BYTE"||o.order=="byte"||
                 o.order=="UBYTE"||o.order=="ubyte"){
            size+=o.args.length();
            ...
            for(int i = 0;i<o.args.length();i++){
                ...
                size+=replaceString(tmp.toUtf8().data()).length();
            }

        }else if(o.order=="ARR"||o.order=="arr"){
            uint size_isSrc,len_isSrc;
            uint size_t = compileUInt(o.args.at(0),&size_isSrc);
            uint len_t = compileUInt(o.args.at(1),&size_isSrc);
            size+=size_t*len_t;
        }
    }
    return size;
...
int searchFreeMemarySpace(bool isCode,QList<uint> base,QList<uint> len,//剩下的物理物理内存区间
    ...
    uint segLength;//段长度。数据已64kb为一段，代码已128MB为一段
    if(isCode){
        segLength = 134217728;
    }else{
        segLength = 65536;
    ...
    foreach(int index,freeMemIndex){
        ...
        uint baseSeg = (base[index])/segLength;
        uint baseShift = (base[index])%segLength;
        uint endSeg = (len[index]+base[index]-1)/segLength;
        uint endShift = (len[index]+base[index]-1)%segLength;

        if(endSeg-baseSeg==0){
            //如果基址和尾址在同一个段，判断该段能不能装下内存块。
            if((segLength-baseShift)>=block_len){
                *block_base = baseSeg*segLength+baseShift;
                return 1;
            }
        }
            ...
            *block_base = (baseSeg+1)*segLength;
            return 1;
        }
    }
...
//判断一个内存块是否跨越了多个内存段
bool isStrideMultipleSeg(bool isCode,uint base,uint length){
    uint segLength;//段长度。数据已64kb为一段，代码已128MB为一段
    if(isCode){
        segLength = 134217728;
    }else{
    ...
    uint base_seg = base / segLength;
    uint end_seg = (base+length-1)/segLength;
 ...
 */
//编译低级汇编文本到内存分配信息表
QList<MemoryMallocInfo> compileASM(QString txt,//编译的汇编文本
                                   QString thisFilePath,//当前汇编文本的文件路径[可填空](用于在包含外部文件时避免重复引用)
                                   QString *errorMessage=NULL,//编译错误提示信息
                                   int * ErCode=NULL,//编译错误码
                                   QStringList *warn=NULL//编译警告信息
                                )
{
    ...
    //物理存储器的基址、长度
    QList<uint> R_CodeMemBase = CodeMemBase;
    QList<uint> R_CodeMemLength = CodeMemLength;
    QList<uint> R_DataMemBase = DataMemBase;
    QList<uint> R_DataMemLength = DataMemLength;
    ...
    for(int i = 0;i<blockList.length();i++){
        ...
        if(blockList[i].length==0){
            warn->append(blockList[i].name+":this block is Empty");
    ...
    if(MemoryArchMode=="HD"){
        ...
        foreach(MemoryBlock block,blockList){
            if((!block.isAuto) && (block.type=="DATA"||block.type=="data")){
                ...
                if(isStrideMultipleSeg(0,block.base,block.length)){
                    if(warn!=NULL){
        ...
        foreach(MemoryBlock block,blockList){
            if((!block.isAuto) && (block.type=="CODE"||block.type=="code")){
                B_base.append(block.base);
                B_len.append(block.length);
                if(isStrideMultipleSeg(1,block.base,block.length)){
                    if(warn!=NULL){
                        warn->append("<"+block.name+">Spanned multiple segments");
                    }
                }
        ...
        foreach(MemoryBlock block,blockList){
            if(!block.isAuto){
                ...
                bool isStride;
                if(block.type=="CODE"||block.type=="code"){
                    isStride = isStrideMultipleSeg(1,block.base,block.length);
                }else{
                    isStride = isStrideMultipleSeg(0,block.base,block.length);
                }
                if(isStride){
                    if(warn!=NULL){
                        warn->append("<"+block.name+">Spanned multiple segments");
    ...
    if(InletCodeBlock!=""){//不为空，说明入口地址通过@Inlet宏确定了
        ...
        if(isStrideMultipleSeg(1,base,length)){
            if(warn!=NULL){
    ...
    if(MemoryArchMode=="HD"){
        for(int i = 0;i<blockList.length();i++){
            ...
            if(blockList[i].type=="DATA"||blockList[i].type=="data"){
                ret = searchFreeMemarySpace(0,DataMemBase,DataMemLength,blockList[i].length,&blockSaveAddress);

            }else if(blockList[i].type=="CODE"||blockList[i].type=="code"){

                ret = searchFreeMemarySpace(1,CodeMemBase,CodeMemLength,blockList[i].length,&blockSaveAddress);
            }
            if(ret==0){
                if(ErCode!=NULL){
                    *ErCode = 19;
        ...
        for(int i = 0;i<blockList.length();i++){
            ...
            int ret;
            if(blockList[i].type=="CODE"||blockList[i].type=="code"){
                ret = searchFreeMemarySpace(1,DataMemBase,DataMemLength,blockList[i].length,&blockSaveAddress);
            }else{
                ret = searchFreeMemarySpace(0,DataMemBase,DataMemLength,blockList[i].length,&blockSaveAddress);
            }
    ...
    for(int i = 0;i<blockList.length();i++){
        ...
        MemoryMark tmpMark;
        tmpMark.markName = blockList[i].name;
        tmpMark.address = blockList[i].base;
        memoryMark.append(tmpMark);
        if(blockList[i].type=="CODE" || blockList[i].type=="code"){
    ...
    for(int i = 0;i<memoryMallocInfoList.length();i++){
        if(MemoryArchMode=="FN"){//冯诺依曼架构
            for(int j = 0;j<R_DataMemBase.length();j++){
                if(memoryMallocInfoList[i].baseAddress >= R_DataMemBase[j] &&
                   memoryMallocInfoList[i].endAddress <= (R_DataMemBase[j]+R_DataMemLength[j]-1)){
                    memoryMallocInfoList[i].devID = j;
                    break;
                }
            }
        }else if(memoryMallocInfoList[i].device==1){//哈佛架构-指令块
            for(int j = 0;j<R_CodeMemBase.length();j++){
                if(memoryMallocInfoList[i].baseAddress >= R_CodeMemBase[j] &&
                   memoryMallocInfoList[i].endAddress <= (R_CodeMemBase[j]+R_CodeMemLength[j]-1)){
                    memoryMallocInfoList[i].devID = j;
                    break;
                }
            }

        }else if(memoryMallocInfoList[i].device==2){//哈佛架构-数据块
            for(int j = 0;j<R_DataMemBase.length();j++){
                if(memoryMallocInfoList[i].baseAddress >= R_DataMemBase[j] &&
                   memoryMallocInfoList[i].endAddress <= (R_DataMemBase[j]+R_DataMemLength[j]-1)){
                    memoryMallocInfoList[i].devID = j;
